...
**Lock freezes the ornament.** While the track is **locked**, the per-cell ornament roll is **suspended and the last-fired ornament repeats deterministically** — Rate still gates *whether* a flourish fires, but Lock fixes *which* one (the engine holds `_lastOrnament` instead of re-rolling). So a good flourish can be caught and held; unlock resumes the random roll. (Lock's other job — write-protecting the trunk — is unchanged. Branches are already deterministic, so Lock doesn't change them.)
...
### KD-19: UI — Setup List + Three-Page Hero Ring

**Decision:** Two surfaces, both standard Performer idioms (renders in `ui-preview/fractal-hero/`).
...
**Goal:** the hero-ring pages defined in **KD-19** (F5 = NEXT). Detail there; the page set:
