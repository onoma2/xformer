...
- **One indexed op.** `TV i` (read) pushes slot `i`'s value; `TV i v` (set) writes it. The slot is a numeric stack arg (0-based, `0..15`) — so a single op, and **computed indices work** (`TV X`, `TV ADD I 1`). Out-of-range index → no-op / pushes 0 (host bounds-checks, like `BUS`).
...
```
Null-guard `h` (null in editor/tests) like every host op. **No macro, no 16 ops** — one function.
...
4. **Vtable** — `table[E_OP_TV] = opTv<Cfg>;` in the registration block of `engine/TeletypeNativeOps.cpp` (`:4179+`).
...
- **Functional via a fake `TT2Host`** backed by a single `int16_t[16]`: `TV 0 42` writes slot 0, `TV 0` reads 42 back; **sharing** — two op invocations against the same fake host see each other's writes; **last-writer-wins** (`TV 0 5` then `TV 0 9` → reads 9); **out-of-range** (`TV 99 …`, `TV -1`) no-ops / pushes 0 (the runtime index now makes this a real path worth testing); `h==null` → get pushes 0. The engine-level "all tracks hit one `_tv`" is sim/manual.

## Budget

Negligible. `Engine._tv` = 32 bytes RAM. +1 op-table slot × 2 configs + 1 small op body × 2 instantiations (`.text`). No per-track/runtime growth.
