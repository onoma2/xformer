...
## Pre-command MODS — COMPLETE (2026-07-06)

Separate from the op table: the pre-command modifiers dispatch in
`TT2Evaluator.h`, not `tt2NativeOpTable`, so they were never in this ledger. As
of 2026-07-06 **all 18 mods are implemented** — `W` (while, cap 10000), `OTHER`
(complements the IF/ELIF/ELSE and EVERY/SKIP chain via the shared
`if_else_condition` flag), `P.MAP` / `PN.MAP` (map a body over a pattern's
`[start,end]` window) were the last four, ported in `TT2Evaluator.h` and covered
by `TestTeletypeV2Mods.cpp`. No mod reaches the `UnsupportedMod` guard anymore.
