...
## Apply models — how a route reaches its target (2026-07-07)

Our engine has **three** apply paths, not one. A route's target picks which:

1. **Override (base + delta)** — continuous per-track/global params (Transpose, Scale, Divisor,
   Tempo…). The route computes a delta from the source × `depthPct` × `shaper`, `combine`
   (Modulate/Absolute) decides base-relative vs absolute, and it lands in the override table
   (`writeRouteOverride`); effective value = `clamp(base + delta)`. The user's hand-set base is
   preserved — routing modulates *around* it. This is `isPerTrackTarget` / `isProjectTarget`.
2. **Trigger (raw source, threshold/edge)** — `Routing::isTriggerTarget` = **Run/Reset** +
   **global transport** (Play/PlayToggle/Record/RecordToggle/TapTempo). These read the raw
   normalized source only: Run threshold, Reset/toggles/TapTempo rising-edge, Play/Record
   level-following. **Depth / scaleSource / shaper / combine do not apply** — the tab editor hides
   those views for trigger rows (renders them SOURCE-only). Consumers: `RouteShellTrigger.h`
   (Run/Reset) and `RoutingEngine::writeEngineTarget` (transport).
3. **Bus (sum-then-clamp)** — CV/MIDI/etc → `BusCv1..4`. Multiple sources **sum** into one bus
   (clamped ±5V). Created on the dedicated **bus tab**, not the param bands. Only a bus routing to
   *itself* is blocked (`isBusSelfRoute`); cross-bus feedback is allowed (bounded, patchable).

### Global transport routing (restored 2026-07-07)
The engine consumer (`writeEngineTarget`, all five transport actions) was built during the matrix
work, but the **UI path to create the routes was never carried over** from legacy. Restored by:
mint ParamKeys 5..9 + `targetToParamKey` cases; fold the five into the **Clock band**; whitelist
them in `tabCellEligible` (engine targets aren't override-params, like Run/Reset); render them as
**global rows** (`!isPerTrackTarget`, since they're engine-scope not project-scope); source-only
via `isTriggerTarget`. `DiscreteMapSync` was *not* restored — global Reset routing replaced it.

### `checkRouteConflict` deleted (2026-07-07)
Legacy wired `checkRouteConflict` into the per-route **Commit** button (block + "ROUTE SETTINGS
CONFLICT WITH ROUTE N"). Our fork's tab editor is **one-route-per-param** (the MOD+ *extend* flow
adds a track to the existing route, never mints a second route to the same param), so the guard's
same-target branch is **unreachable** — and that branch would wrongly block legitimate **bus
mixing** (many sources → one bus). Its transport-exclusivity branch (Play↔PlayToggle,
Record↔RecordToggle) *is* reachable now, but routing both is a rare, non-destructive, recoverable
patch (Play's level-following wins steady-state; worst case is transport stutter, fixed by
deleting one route). Not worth wiring a guard for — the dead function was deleted.
