...
- **Input conditioning — attenuversion + bias (harmonàig input stage):** two fields scale the raw
  source *before* quantise. **Attenuvert** — a signed gain (bipolar, ~`−2…+2`, a hair past unity like
  the harmonàig knob) multiplies the CV: inverts polarity, shrinks or stretches the melodic range.
  **Bias** — a volt offset added after the gain, shifts the whole input window (a bias in whole
  semitones is the chromatic input transpose harmonàig's Global Offset gave, which the in-scale
  `Transpose` §3a can't). `quantiseInput = raw × attenuvert + bias`, then the §2g quantise. Storage:
  two plain `int8_t` members read through `Routing::routedValueInt(ParamKey::…)`, mirroring CurveTrack's
  `_shapeProbabilityBias` (`CurveTrack.h:160`) — **not** a `Routable<T>` wrapper. Per-pattern setup (§11),
  routable via the §6 harmony block so a CV can sweep range or transpose the input live.
...
(diatonic degrees only), and a `note % 7` degree hack (`NoteTrackEngine.cpp:993`, comment "for
...
- **Degree Rotate** — *per-step + routable*. Rotates the harmonised degree over the **12 chromatic
  ...
  Stochastic's `degreeRotation` (`StochasticSequence.h:243`) — but that rotates modulo *active scale
...
- **Transpose** — *track-level*. Performer's diatonic scale-step transpose (`evalTransposition`,
  `NoteTrackEngine.cpp:71`) — the harmonàig "Global Offset" role. Shifts the whole output, stays in
...
**Inversion, Voicing, Quality, Degree Rotate, Strum, Mask Rotate** (§2g), **Attenuvert, Bias** (§1)
(the overall Transpose rides the standard track-transpose routing and Slew rides the standard `SlideTime`
routing, neither in this block). A routed value **offsets the per-step base** (same convention as other tracks), so a CurveTrack/CV can
...
- **Slew (global, §11):** per-voice CV portamento — each voice glides to its new pitch on a chord
  change. Reuses the standard track slew: `slideTime` (plain `uint8_t`, 0–100%, routed via
  `routedValueInt(ParamKey::SlideTime)`, `NoteTrack.h:121`) feeding
  `Slide::applySlide(cur, target, slideTime, dt)` (`engine/Slide.h`) per voice in `update()`. NoteTrack
  gates that call on a per-step `_slideActive` flag (`NoteTrackEngine.cpp:504`); harmony has no per-step
  slide bit, so it glides unconditionally whenever `slideTime > 0`. One value for all four voices (mirrors
  gate length); `0` = off (stepped). Rides the standard `SlideTime` routing (§6), not a new target.
  Orthogonal to strum:
  strum staggers onset *timing*, slew glides the *pitch* each voice lands on. JI (§3b) retunes the
  target; slew glides toward the retuned value.
...
- **Source read — one-tick-settled, lag accepted:** the engine reads its source via `cvOutput()` /
  `channel()`. Engines tick in fixed index 0→7 order with output composition batched *after* the loop,
  so a source with a **higher** index than the harmony track returns last tick's value
  (`docs/performer-architecture.md:610-612`) — there is no run-after-a-chosen-source hook. This one-tick
  lag is **accepted**: inaudible for the delta-auto S&H (§4) and identical to the old NoteTrack harmony.
  Edge-detection (`prev quantised note`) compares against the previous settled sample.
- **Transport / reset (Stochastic pattern):** `reset()` (Stop/Start) returns the recipe cursor to
  `sequence.first()` and clears held voices, gates, prev-quantised-note, the Sleep counter, and
  per-voice slew state — mirrors `StochasticTrackEngine.cpp:163`. `resetMeasure()`
  (`sequence.resetMeasure() × measureDivisor`, `:145/:211`) snaps the cursor to first on the measure
  boundary. `Continue` resumes with no reset. In keyboard mode (§11) the cursor is frozen, so
  transport advance is suspended while the Play page is active, but Stop still clears held voices.
...
- **Trio:** `model/HarmonyTrack.h`, `model/HarmonySequence.h`, `engine/HarmonyTrackEngine.h` —
  standard NoteTrack/CurveTrack precedent. Reuses `model/HarmonyEngine.h`'s inversion/voicing/interval
  helpers, but its 4-field tertiary `ChordNotes` container is reshaped to an N-voice array (§10).
...
- **`engine/NoteTrackEngine.cpp`:** `applyHarmony()` (~`:929`) + its three call sites
  (~`:758/:823/:844`), the `#include "model/HarmonyEngine.h"`, and the `% 7` degree hack (~`:993`).
...
**Keep:** `model/HarmonyEngine.{h,cpp}` `applyInversion`/`applyVoicing`/`applyInterval` helpers — the
harmony track reuses them (§9); its quality enum + `DiatonicChords` table are replaced (§2). **Caveat:**
those helpers assume a fixed 4-field tertiary `ChordNotes{root,third,fifth,seventh}`; generic voices
(§7), 3-note sus chords (§2a), and generative stacking (§2e) need that container reshaped to an N-voice
array — a reshape, not just an enum+LUT swap. `model/Model.h`'s reference follows whatever `HarmonyEngine`
becomes.
...
- Create the trio; add `TrackMode::Harmony` to the `TrackMode` enum (`Track.h:44`) (enum value only).
...
- Add input **Attenuvert + Bias** fields (§1, `CurveTrack.h:239` `Routable` precedent) applied before quantise.
- Add per-voice **Slew** — reuse `slideTime` + `Slide::applySlide` (§7); apply in the engine `update()` per voice.
- Add the **Play** keyboard hero page (§11) ported from the modulove `KeyboardPage` button map; a key press drives the §4 root-inject path.
...
- UI infrastructure (§17): ClipBoard `HarmonySequence` hooks, context menu, quickEdit, USB keyboard baseline.
- Transport/reset in the engine (§8) — `reset` / `resetMeasure` / `restart` mirroring `StochasticTrackEngine`.

---

## 11. UI surfaces

- **HarmonySequenceEditPage — four hero pages, F5 = NEXT** (Stochastic ring: internal `Page` enum,
  `nextPage()` wraps `% Page::Count`; F5 cycles Recipe → Scale → Chord → Play):
  ...
  - **Play** — a playable one-octave keyboard (harmonàig Performance Mode), modelled on the modulove
    fork's `KeyboardPage` (`temp-ref/modulove-performer/src/apps/sequencer/ui/pages/KeyboardPage.cpp`):
    the 16 step buttons are a piano — white keys **S9–S16**, black keys **S2/S3/S5/S6/S7** — encoder or
    **Left/Right** shift the root a semitone, **F octave ±**. A key press **injects that note as the
    harmony root**, bypassing the CV source: it re-quantises + re-voices on the played note, harmonises
    it with the **current** recipe step (or Chord-page live override), and gates the chord out across the
    voices; release drops the voice gates. **Entering keyboard mode freezes the recipe cursor
    altogether** — while the Play page is active no source advances it (not the played keys, not the
    assigned §4 trigger); the chord recipe is whatever step the cursor sits on, and the keyboard is the
    sole root+gate source. Leaving the page restores normal §4 trigger-driven advance. Draws the piano +
    held note like the ref; footer ends `… · NEXT`.
- **SequencePage (list, per-pattern):** Root / Scale / Mode (`scaleRotate`), **CV Source** (Track 1–8 /
  CV In 1–4), **Attenuvert** (input gain, §1), **Bias** (input offset, §1), **Trigger Source**
  (delta-auto / gate / internal counter), **Gate Length** (`0` = T, `1–100%`), **Mask Rotate** (§2g,
  routable), divisor, runMode, first/last step.
- **TrackPage (setup, track-global):** `Transpose` (standard diatonic, §3a), `Slew` (`slideTime` %, §7),
  `Tuning` (ET/JI, §3b).
...
- **A0 — Serialization round-trip (first test, retro #4).** write → clear → read → assert equality on
  every `HarmonySequence` field + `Step` record (recipe fields, `noteMask`, scale group, source/trigger/
  gate-length/attenuvert/bias/slew). No feature work proceeds until it passes — catches missing fields,
  wrong order, wrong defaults, enum-clamp bugs.
...
## 15. State ownership (Track / Sequence / Engine / UI)

Locked before code (retro #1). Each field has exactly one owner. Scope mirrors the Stochastic borrow
pattern — track-global chassis on `HarmonyTrack`, per-pattern config + recipe on `HarmonySequence`.

| Field | Owner | Notes |
|-------|-------|-------|
| `scale` + `rootNote` + `scaleRotate` (mode) | **Sequence** | bias-packed `uint16` group (§2f), per-pattern |
| `noteMask` (32-bit chromatic) | **Track** | one keyboard mask across patterns (§2g) |
| `maskRotate` | **Sequence** | routable offset (§2g), per-pattern |
| CV Source, Trigger Source | **Sequence** | per-pattern (§4, §11) |
| Attenuvert, Bias | **Sequence** | plain `int8_t`, routed via `ParamKey` (§1) |
| Gate Length | **Sequence** | one value, all voices (§7) |
| divisor, runMode, first/last step | **Sequence** | standard sequence fields |
| Per-step recipe `Step` (Quality/Inv/Voic/DegRot/Strum/Octave/Gate/Sleep) | **Sequence** | bit-packed 32-bit word (§5) |
| Transpose | **Track** | standard track transpose (§3a) |
| Slew (`slideTime`) | **Track** | plain `uint8_t`, `ParamKey::SlideTime`, all voices (§7) |
| Tuning (ET/JI) | **Track** | setup field (§3b) |
| recipe cursor, `_sleepRemaining`, prev-quantised-note | **Engine** | transient; cleared on reset (§8) |
| per-voice `_cvOutput`/`_cvOutputTarget`/`_gateOutput`, strum onset deadlines | **Engine** | transient output state |
| live Chord-page overrides (latched), keyboard-mode + held key | **Engine** | runtime; `Follow` steps defer to these (§2d, §11) |
| JI log2-ratio volt offsets | **Engine (const)** | precomputed at init (§3b), not per-tick |

## 16. Retro lesson gates

10-row map to `retro/stochastic-retro.md` (retro #-cited inline elsewhere).

| # | Lesson | How MVP satisfies |
|---|--------|-------------------|
| 1 | State ownership before code | §15 table, locked |
| 2 | Audit ref for embedded incompat | JI/quantise are LUT + precomputed `log2` (§3b); strum on `os::ticks()` wall-clock; no `std::vector`/heap/`time()`. No RNG in the harmony math/quantise/voicing; cursor traversal under `Random`/`RandomWalk` runMode reuses the standard shared `Random` (NoteTrack precedent) |
| 3 | `sizeof` on STM32 target | §9 fit-check measured on the ARM release build vs PROJECT.md caps, not host |
| 4 | Serialization round-trip first | §14 A0 |
| 5 | Integer-division cliffs | strum-time ms, gate-length/attenuvert %, slew reuse `adjustedByStep`/`editSlideTime` (×10-before-divide, step 5) |
| 6 | Single frozen vocabulary | CANON / alt / recipe / voice / mask frozen; terms tagged (code)/(ui)/(idea) |
| 7 | Validate identity/storage model | recipe-replay locked (§2/§5): per-step recipe applied to a freshly quantised root, **not** stored output |
| 8 | Cache invalidation single owner | a mode/mask/override change invalidates via one re-quantise+re-voice on the next trigger — no distributed setters |
| 9 | Adversarial review early | run `ce-*-reviewer` after model/engine lands, again after the output pipeline |
| 10 | Consolidate design docs | this spec is the single current doc; legacy NoteTrack harmony marked for teardown (§10); scale-rotate deps cited, not chained |

## 17. UI infrastructure surfaces (NoteTrack baseline)

Beyond the four hero pages (§11), the EditPage owes the standard infrastructure (template lesson #22).
All model on the PhaseFlux precedent.

- **17.1 Clipboard hooks.** Extend `model/ClipBoard.{h,cpp}`: `copyHarmonySequence` /
  `pasteHarmonySequence` / `canPasteHarmonySequence` + a `Type::HarmonySequence` enum entry + the
  `HarmonySequence` include in the Pattern union — pattern-level copy/paste across tracks then works via
  `PatternPage` automatically. Three ~4-line methods following `ClipBoard.cpp:62-71, 230-247, 322-340`.
- **17.2 Context menu — INIT / COPY / PASTE / DUP.** `key.isContextMenu()` (PAGE+Shift) →
  `contextShow()`. `HarmonyContextAction { Init, Copy, Paste, Dup, Last }`. INIT opens a sub-menu:
  **Step** (selected cell `clear()`), **Sequence** (`HarmonySequence::clear()`), **Track**
  (`HarmonyTrack::clear()`). PASTE greyed via `canPasteHarmonySequence()`. DUP = copy → advance pattern →
  paste → clear clipboard. Precedent `PhaseFluxEditPage.cpp:716,728`.
- **17.3 QuickEdit — PAGE+S8..S15.** `key.isQuickEdit() && !key.shiftModifier()`. Slot 4 (S13) INIT,
  Slot 6 (S15) RANDOM (randomise the recipe), Slot 7 (S16) page-scoped variant; other slots free.
  Handlers shared with the context menu. Precedent `PhaseFluxEditPage.cpp:315`. On the **Play** page the
  step buttons are piano keys — the PAGE modifier keeps quickEdit unambiguous.
- **17.4 USB keyboard baseline.** Two-line override — `if (handleFunctionKeys(event)) return;` then
  `BasePage::keyboard(event);` — for F1–F5 + Tab (context menu) + arrows (nav/encoder). The Play page's
  note-key handling (§11) layers on top. Precedent `PhaseFluxEditPage::keyboard`.
