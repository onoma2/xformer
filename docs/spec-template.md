...
11. **Stochastic borrow pattern** for track-level chassis: copy `_slideTime / _octave / _transpose / _playMode / _fillMode / _cvUpdateMode` verbatim with Routable wrappers; drop probability biases if no probability fields exist. **A borrowed field is dead unless the engine reads it.** `_cvUpdateMode` shipped on Fractal with model + UI but the engine never checked it, so the knob did nothing — CV always updated on rests. The engine must branch on it in the gate-off path: write CV on an ungated step only when mode == `Always`, else hold the previous pitch (`NoteTrackEngine.cpp:836`). `CvUpdateMode::Last` is the enum count terminator, not a mode — only `Gate` / `Always` are selectable.
...
23. **Step LEDs are on/off only.** Paint with `leds.set(index, red, green)`; amber (both) = selection, green = active, red = inactive, off = unused (`LedPainter.cpp:14-17`). Never use `leds.setDimmed()` with an intermediate intensity to mean "dim/present" — the matrix driver renders low intensities with a 4-bit sigma-delta PWM that reads as constant flicker, not steady dimming (`ButtonLedMatrix.h:86-95`). Carry a second state with a different colour, not with brightness.
