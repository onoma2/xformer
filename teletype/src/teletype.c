...
tele_error_t validate(const tele_command_t *c,
    ...
    while (idx--) {  // process words right to left
        tele_word_t word_type = cmd_tag(c, idx);
        int16_t word_value = cmd_value(c, idx);
        // A first_cmd is either at the beginning of the command or immediately
        // after the PRE_SEP or COMMAND_SEP
        bool first_cmd = idx == 0 || cmd_tag(c, idx - 1) == PRE_SEP ||
                         cmd_tag(c, idx - 1) == SUB_SEP;
        ...
        if (word_type == NUMBER || word_type == XNUMBER ||
            ...
            else if (word_value == E_OP_BAR || word_value == E_OP_WMS) {
                if (stack_depth > 1) {
                    strcpy(error_msg, op->name);
            ...
            if (cmd_tag(c, 0) != MOD) return E_PLACE_PRE_SEP;
...
process_result_t process_command(scene_state_t *ss, exec_state_t *es,
    ...
    // iterate through command words to find all the SUB_SEPs and add to the array
    ...
    for (ssize_t sub_idx = 0; sub_idx < sub_len && !es_variables(es)->breaking;
        ...
        for (ssize_t idx = sub_end; idx >= sub_start; idx--) {
            const tele_word_t word_type = cmd_tag(&c, idx);
            const int16_t word_value = cmd_value(&c, idx);
