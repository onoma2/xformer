...
void copy_post_command(tele_command_t *dst, const tele_command_t *src) {
    ...
    dst->separator = -1;
    memcpy(dst->tag, &src->tag[src->separator + 1],
           dst->length * sizeof(dst->tag[0]));
    memcpy(dst->value, &src->value[src->separator + 1],
           dst->length * sizeof(dst->value[0]));
}

void print_command(const tele_command_t *cmd, char *out) {
    out[0] = 0;
    for (size_t i = 0; i < cmd->length; i++) {
        tele_word_t tag = cmd_tag(cmd, i);
        int16_t value = cmd_value(cmd, i);
        ...
        if (i < cmd->length - 1) {
            ...
            tele_word_t next_tag = cmd_tag(cmd, i + 1);
