...
Phase 2b candidates:
- **`DEL n: body` (one-shot ms) + `DEL.CLR` — DONE (`4977d8af`).** Faithful: parallel-slot queue, `<1→1` clamp, origin snapshot, ms drain from `update(dt)`, issue-80 busy-slot guard. `TestTeletypeV2Delay` 7 cases. **Remaining delay: `.X/.R/.G/.B` repeat variants** (interval/count/geometric/bitmask — formulas in `teletype/src/ops/delay.c`).
