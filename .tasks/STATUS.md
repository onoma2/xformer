# Task Board
_Updated: 2026-05-22 (stochastic: ownership-contract Patches 1+2 landed — const_cast dropped, intent-named helpers extracted; STM32 + four test suites clean)_
...
**Where I stopped:** Review found implemented-risk items: engine mutates model sequence state from the engine task, lock replay cache is not pattern-scoped, stochastic pages lack local type guards, PlayMode is exposed but not implemented in timing, lock heap allocation can fail silently, Lock persistence needs an explicit decision, and invalid serialized PlayMode defaults to Free while `clear()` defaults to Aligned. Ownership-contract **Patches 1+2 landed 2026-05-22**: (P1) engine stores `StochasticTrack&` (non-const) and `StochasticSequence*` (non-const), all 9 `const_cast` sites removed; (P2) added local `sequence()`/`stochasticTrack()` accessors and extracted intent-named private helpers — `ensureLoopSources`, `writeLiveRhythmShadow`, `writeLiveMelodyShadow`, `captureLockedParent`. No behavior change. STM32 release builds clean (`.text=889,080`, +1,868 B vs pre-Patch-1 baseline). Sim + four stochastic test suites pass. Phase 13 feature research + generic-track gap audit captured in `.tasks/stochastic-track-port/PHASE13-FEATURES-PLAN.md` — batched 0 (correctness gate: ownership + lock cache + page guards + PlayMode/FillMode truth + lock alloc feedback + Mutate readout), 1 (clarity UI), 2 (workflow gaps), 3 (engine feel: Slide harmonize + Strum + RunMode), 4 (long tail).
**Next action:** Continue Batch 0: lock cache pattern-scoping (clear `_lockedParents` + queues on `changePattern()`), page type guards (`if (trackMode() != Stochastic) return;` at draw/LED/keyDown/keyPress/keyboard entry), PlayMode/FillMode truth pass (hide both from config until wired), invalid-PlayMode default fix (`read()` Aligned to match `clear()`), lock allocation feedback. Hardware verify after Batch 0 complete.
...
**Next action:** Phase 0 contract/stage-gate cleanup: reconcile `TASK.md` + `DICTIONARY.md`, define trunk/gate/capture/window/reset semantics, mark active vs reserved fields, then resume Phase 1 model work with STM32 sizeof probe.
