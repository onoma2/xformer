...
**Status:** Phase A + Phase B + Phase-C hero edit + per-stage stageLen + per-sequence globalPhase + gateLength rules + cvUpdateMode Always + PitchMode (Cell/Global) + pitchRate landed. STM32 release build (last verified) clean, both static_assert ceilings pass.
**Where I stopped:** Accumulator v2 spec locked at `docs/accumulator-v2-spec.md §13`. Dual accumulator design: Note (drifts pitch degree) + Pulse (drifts pulseCount), both per-sequence config (Scope/Order/Polarity/Reset/+Lim/-Lim) + per-cell step + per-cell trigger. Shared `AccumulatorConfig` + `AccumulatorOps::applyOrder` engine (8 Order×Polarity cells in pseudo-code). Reset is an integer 0..15 (0=manual, N>0=auto every N cycles — "integer for implicit auto mode"). Hero edit page extends carousel to TEMP/PTCH/ACCUM.N/ACCUM.P with dual-strip visualization (4-grouped bars, Color.Low dotted gridlines, no midlines, active layer bright). Slot map: AMOUNT/+LIM/-LIM/ORDER/RESET plain, Shift+F1=TRIG / Shift+F4=POLAR / Shift+F5=SCOPE. Bit-pack: reclaims `accumulatorLength`'s 4 bits, adds 6 new per-cell bits — no Stage size growth. "Counter decoupled from output" universal rule covers all open questions. §13.9 pseudo-code for applyOrder, §13.10 = 10 implementation tasks with deps, §13.11 = concrete unit-test names per task. Renders at `ui-preview/phaseflux-accum/`. Spec is subagent-driven-development ready. Previous landing (pitchMode + pitchRate) ready for hardware audition separately.
**Next action:** Implement accumulator v2 per `docs/accumulator-v2-spec.md` §13.10 task list — start with Task 1 (`AccumulatorConfig` struct + roundtrip test). Subagent-driven-development can dispatch the 10 tasks; Tasks 1-4 are largely parallel after Task 1 lands, Tasks 5-9 depend on the model layer, Task 10 is the audible-verification gate. Hardware smoke test of pitchMode + pitchRate can run in parallel before accumulator work merges.
**Depends on:** nothing
