# Task Board
_Updated: 2026-05-23 (Phase 14B Patches A/B/C step 1/1b/2 landed + hardware-verified; Phase 16 drafted — flat cell model with Feel knob replaces the parent/child concept; mutation log + Repeat baked + events[] deletion folded into Phase 16's P7/P8)_
...
**Next action:** Land Phase 16 in eight patches: **P1** queue contract audit, **P2** cell packing (steal octave bit for audible, drop parentCacheIdx, shrink aux to 1 B → 5 B/cell), **P3** bump kCellCap 64→80 (net −368 B CCMRAM with P2's packing), **P4** Feel knob (stored, inert), **P5** flat generator rewrite (the big one — interval walk + cluster-as-duration-bias + Feel scaling pass), **P6** CycleEndPlan struct, **P7** mutation log + Repeat baked + seed-lock guard, **P8** delete `events[]` and `StochasticSourceEvent`. Each patch test-gates on all 6 stochastic suites + STM32 release before commit. P1-P3 are pure cleanup; P4-P5 is the flat migration; P6-P8 completes Phase 14B's identity work under the cleaner foundation.
