...
**Where I stopped:** On `refactor/routing-matrix`: U1 `RouteParam` (+fail-closed guard), trigger unification (Reset/Play/Record/TapTempo → one `RouteState.gateMask`). **Shared `ParamKey` registry landed** (`model/RouteParamKey.h`, single append-only numbering authority, F6) and **U2 Global + U3 Note tables reworked onto it** (dropped their local `enum Key`; rows reference `ParamKey::`). `TestParamRegistry` guards table-row key uniqueness (Codex review hardening — `Table::find` returns first scan hit, so a dup would silently alias). writeTarget-parity green; sim + STM32 release clean; nothing wired live. **Param triage RESOLVED:** launch = routable-today + Phase + PhaseFlux A/B inlets + five nudges + two defect-fixes (StochasticFeel, Stoch/PF Scale-RootNote base-write); Teletype out; SlewTime≡SlideTime; rest deferred. Codex U7 watch-item: `uint8_t` key width settled at cutover not widened now (~130–160 keys fit). Full snapshot in TASK.md.
**Next action:** U4 — second per-type table (proposed Curve: max shared-key reuse with Note, introduces Phase Tier-2b + DSP block) appended to the registry + uniqueness sweep → inlets (Indexed A/B, DiscreteMap In/Scanner/Sync) → U5 scaleSource, U6 groups, U6b override+combine (range-class lands here), U7 cutover.
