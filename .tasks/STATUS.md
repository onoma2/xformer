# Task Board
_Updated: 2026-06-04 (routing-matrix: **slice-6 tab editor is now PARAM-CENTRIC**. Model half DONE (9885d5aa→997f8b84→d8a4a16b). UI chain: route overview (ee2f6514) → engine honors combine (96c08e17) → tab shell (e601fa36) → editable depth/combine (ac589d48, **HW-verified**: saves/loads + combine switches live) → **RouteBrowse read model** (eb659fba, band→params + scope match, TDD) → **band aggregation display** (8ed4ab1c + 6e5a23c2 focused-row live-depth) → **per-row cursor** (73926bd5 + 2c409112 auto-save/scope fixes). The tab editor (Page+S6) now navigates a band's params: encoder=cursor, press=edit toggle, turn-edit=depth, F2=combine, F5=commit, Left/Right=band; edits auto-save on navigate. Sim/debug at **zero warnings** (46fb2d0d + 7b8892a3). All units Codex-gated. Reality check: navigates + edits EXISTING routed params (depth/combine, live); unrouted rows show "--" and are display-only (creation = next unit); source/target/tracks still via OLD RouteListModel rows; scope/spread/target lenses are ui-preview mockups. Model commits: 9885d5aa→997f8b84→d8a4a16b. UI/nav spec `docs/plans/2026-06-03-009-routing-ui-nav-spec.md` (Codex-validated) + tier/lens design `007` written; routing-design renders in ui-preview/. Decision: Global is base+delta, combine picks Modulate/Absolute, first UI pass = Note+PhaseFlux+Global. Prior: slices 1-4 + editable-base (plan 006) landed; **slice 5 hardware audition PASSED** — Note/PhaseFlux override path verified on hardware. Prior: sliced cutover plan 005 — slices 1-4 landed TDD+Codex-clean. The cutover is COMPLETE: Note+PhaseFlux per-track routing now runs the override path end-to-end. Slice 4 migrated all 22 getters to clamp(base+override) + gated every interactive knob-edit path on override presence; PhaseFlux Scale/RootNote base-write defect fixed. Live Note/PhaseFlux routing restored (was inert mid-slice). Next: slice 5 = hardware audition.)_
...
**Next action:** Slice 6 UI — param-centric cursor done (73926bd5). **Now: route creation on an unrouted row** — press-to-create on a `--` row binds a fresh route (target from the cursor's band key via a key→Target reverse, tracks from `_tabScopeMask`, allocate via findEmptyRoute), so a new param can be routed from inside the tab editor instead of the old rows. Then: source editing in-editor, Page+S5 spread (per-track unique depths), engine-tab for the mode's own band; finally promote the tab editor over the old RouteListModel for migrated/global targets. scaleSource cross-source resolution is its own later unit. First pass exposes Note+PhaseFlux+Global only; the 6 unmigrated engines keep old rows. Then 7 (other six engines + their UI tabs, paired) → 8 (U7 serialized format) → 9 (delete old writeTarget/Routable routed half). **Carry-forward:** Python `Project.tempo`/`swing` bindings still getter+base-setter (latent read-modify-write absorb; no live caller) — add base-getter binding when the Python surface is next touched.
