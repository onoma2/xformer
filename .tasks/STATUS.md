...
**Phase 1 (spec 013 §12) — model half DONE.** Plan `docs/plans/2026-06-04-014-modulation-phase1-plan.md`. **DESIGN AMENDMENT (spec 013 §15):** the context ROUTE slot becomes ONE state-dependent slot **MOD+** (this track not modulated → add/create) / **MOD-** (this track modulated → remove this track, free slot when last) — resolves the 5-slot-full collision (REMOVE MOD overflowed; 3 of 4 migrated pages full) and per-track-scoped removal. Labels render-checked (`ui-preview/modulation/modulation-ctx-*.png`, MOD+/MOD- = 22px). **Task 1 DONE (Codex-gated):** `model/RouteDraft.h` draft/commit staging (`Draft`+`create`/`begin`/`canCommit`/`commit`/`cancel`); live untouched until COMMIT; cancel frees a new slot; source+active+routeIndex-bound required to commit; create rejects None target / out-of-range track; absorbs depth-0-on-create. **Task 2 DONE (commit e2c00d16, Codex-gated):** membership ops `isTrackModulated` + `removeTrack` (per-track: clear bit + zero depth, free slot if last; global: free; bounds-guarded). `TestRouteDraft.cpp` 16 cases green; sim clean (only pre-existing teletype/libcore warnings). **Next (UI, owner hardware-verify): Task 3** state-dependent MOD+/MOD- context slot on migrated pages (Note/PhaseFlux/Project) via `isTrackModulated`; **Task 4** wire MOD+ (RouteDraft::create→source→commit) / MOD- (removeTrack) handlers; **Task 5** migrated-only gating + hide non-migrated ROUTE entries; **Task 6** retire legacy showRoute/RouteListModel + MVP beginModulate auto-chain.
