# Task Board
_Updated: 2026-05-21 (stochastic hero pages live in firmware; sim-verified, hardware re-flash pending)_
...
## 🟡 stochastic-track-port — Hero pages live in firmware; Burst math + Direct hero responsiveness deferred
**Status:** paused — Phase 2 complete and hardware-stable; hero pages (CORE / MARBLES / DIRECT / LOOP) shipped in firmware via `StochasticSequenceEditPage` extension, page-key routing matches other-track conventions, duration LUT cleaned. Sim runs; hardware re-flash needed to verify.
**Where I stopped:** Hero pages landed across 5 commits (372c0400, 4d994491, eabad1e2, 4a9ff36f, 94c915fc). One step = one continuous param + Fn keys for toggles, encoder writes held step. Loop page tape redesigned IndexedSequenceEditPage-style with adaptive event width + pitch-height fill. Footer labels reflect live state (LOOP/LIVE/SPLIT, MARBLE/SHAPE, LOCKED/FREE). Page+S0/S1/S2 route to hero edit / sequence list-model / track list-model. Duration LUT now `[1/2, 1/4, 3/16, 1/8, 1/8T, 1/16, 1/16T, 1/32]` sorted descending by ticks. Phase 2 crash fixes (Engine `updateTrackSetups` order, Container destructor pointer) still in place. SRAM 90.7%, `.text` 859 KB, ~10 KB grew from hero work, zero new RAM. STM32 release builds clean.
**Next action:** (1) Direct page hero responsiveness — drawing currently only reacts to live engine CV, not to held-step L2 param edits (variation/tilt/contour/rate/linearity/rest/slide/jump). Add per-step visual response so user sees encoder edits land. (2) Burst math rework (Cluster F) — Burst at short parent durations produces mush; needs duration gate or different model. (3) Hardware verification of hero pages: SHAPE round-trip, MODE toggle, loop-tape pitch read, page-key routing. (4) Hardware-tuning steps still pending: Marbles scale-aware Steps law, bipolar Mutate semantics, Density/Complexity derivation curves. (5) UX call on dual Character/Complexity list entries.
**Branch:** feat/stochastic
**Reference:** `.tasks/stochastic-track-port/PHASE10.7-L1-REDESIGN.md`, `.tasks/stochastic-track-port/PHASE10.6-BUG-CATALOG.md`, `.tasks/stochastic-track-port/UI-CONTROL-DRAFT.md`, `ui-preview/pages_levels.py`, `ui-preview/render_levels.py`, `ui-preview/level-previews/`
**Branch:** feat/stochastic
