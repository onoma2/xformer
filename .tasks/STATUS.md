...
**Phase 1 (spec 013 §12) — IN PROGRESS.** Plan `docs/plans/2026-06-04-014-modulation-phase1-plan.md` (Task 1 = RouteDraft core, then UI rename/gate/retire). **Task 1 DONE (commit 8197ea0d, Codex-gated):** `model/RouteDraft.h` — header-only draft/commit staging both doors share (`Draft`=working Route copy+slot index+isNew; `create`/`begin`/`canCommit`/`commit`/`cancel`). Live route untouched until COMMIT; cancel frees a freshly-created slot; source required to commit (+ `active()` target-None guard); create rejects None target / out-of-range trackIndex. Absorbs depth-0-on-create. `TestRouteDraft.cpp` 8 cases green; sim sequencer clean. **Next: Task 2 — rename context action ROUTE→MODULATE on migrated pages (Note/PhaseFlux/Project), render-check "MODULATE" fits the context menu BEFORE firmware; then Task 3 REMOVE MOD, Task 4 migrated-only gating + hide non-migrated entries, Task 5 retire legacy showRoute/RouteListModel path.** Tasks 2-5 are UI; need hardware-verify by owner.
