# Task Board
_Updated: 2026-05-22 (Phase 14B seed+log chosen as direction; Phase 14 trunk model retained as considered alternative; design only, no code; new branch feat/stochastic-seed-log created for the work)_
...
## 🟡 stochastic-track-port — Phase 13 cleanup landed; Phase 14 trunk-model design drafted
**Status:** paused — STM32 `.text=885,972`, `.data=6,416`, `.bss=167,996`. Four stochastic test suites pass. Hardware-verified through 8 stochastic tracks (no crash, snap-flush working).
**Where I stopped:** Phase 13 shipped end-to-end: ownership contract + UI overhaul (`6309ed8d`, `a2208039`), heap-cache crash fix (`788fdd5a`), alias + routing cleanup (`ff089745`, accent removed, fields renamed to actual semantics, routing enums + labels aligned, snap-flush bug fixed). Then a research pass on what other generative sequencers store-vs-re-roll (Bloom v2, Marbles, Turing, PNW, Melodicer, SiG, Hemisphere) revealed our "what gets stored" line is drawn at byte-budget locality rather than musical identity — Bloom + community consensus stores gate length, slew, ratchets per step as identity, treats classical music flourishes (mordent, trill, arp) as the orthogonal ornament axis. This led to drafting **Phase 14 — Trunk Model** (`.tasks/stochastic-track-port/PHASE14-TRUNK-MODEL.md`): collapse the per-event recipe struct into a baked CV/gate cell array (the "trunk"), make playback a 30-line tape pumper with no LUT lookups, let the generator be the only thing that knows about upstream knobs. Lock becomes a single bit on the sequence/track that refuses generator writes — `LOCK-DESIGN-DEFERRED.md` becomes a one-paragraph note in Phase 14. Design only, no code yet. 8 open product questions captured in the doc (per-track vs per-pattern trunk, cell pack bytes, trunk cap, Repeat baked vs live, Live mode semantics, Mask/Tilt rank caching, burst-cell flag, walker source).
**Next action:** Work seed+log on `feat/stochastic-seed-log`. Foundational validation first: per-cell keyed RNG test — prove cell N's output depends only on `(seed, N, knobs)`, not on stream consumption order. After that invariant holds, resolve the 7-point acceptance bar in PHASE14B (lock-freezes-across-knobs, keyed RNG, repeat baked, live-regen-cadence feel, project migration loss, cache CCMRAM-only, UI guards on cache availability). Then Patch A: engine cache plumbing alongside existing event tape, no behavior change. In parallel on `feat/stochastic`: finish Phase 13 UI cleanup (Mutate readout `PERM/REGEN/HOLD`, source-state header chip, PlayMode/FillMode hide-or-implement) for a stable release. Conceptual frame: today's Loop/Live toggle = seed lock (recipe freeze, knobs still shape output); deferred Lock = content lock (engine cache snapshot, fully frozen).
...
**Reference:** `.tasks/stochastic-track-port/PHASE14B-SEED-LOG.md` (drafted 2026-05-22, **chosen direction**), `.tasks/stochastic-track-port/PHASE14-TRUNK-MODEL.md` (drafted 2026-05-22, considered alternative — retained for reference), `.tasks/stochastic-track-port/LOCK-DESIGN-DEFERRED.md` (folded into Phase 14B as content-lock), `.tasks/stochastic-track-port/PHASE13-FEATURES-PLAN.md` (cleanup history), `docs/stoch-review.md` (adversarial review), `.tasks/stochastic-track-port/PHASE11-SIMPLIFY.md` (drafted 2026-05-21), `.tasks/stochastic-track-port/PHASE7-DICTIONARY.md` (canonical), `.tasks/stochastic-track-port/PHASE10.7-L1-REDESIGN.md`, `.tasks/stochastic-track-port/PHASE10.6-BUG-CATALOG.md`, `.tasks/stochastic-track-port/UI-CONTROL-DRAFT.md`
