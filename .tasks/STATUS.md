...
**Status:** Accumulator v2 + ACCUM hero redesign + TEMP/PITCH scope visuals + 4+Next paged footer across all topics + **Repeat & Window per-axis curve transforms wired end-to-end** (data model + engine + UI P2 page + scope rendering + unit tests). Engine pipeline `Window → Repeat → Warp → curve → FlipV → Response → FlipH`. Sub-section algorithm for temporal axis; fmod-Repeat for pitch.
