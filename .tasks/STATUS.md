...
**Where I stopped:** On `refactor/routing-matrix`: U1 `RouteParam` (+fail-closed guard), trigger unification. **Shared `ParamKey` registry** (`model/RouteParamKey.h`, append-only authority, F6); **tables on it: Global, Note, Curve, Indexed, DiscreteMap, MidiCv, Tuesday** — all Codex-reviewed. Curve validates "one key, many types"; Indexed established the INLET row-kind (A/B); DiscreteMap owns Input/Scanner/Sync inlets + SlewTime≡SlideTime resolved + snapshot-fan-out parity fix; MidiCv 2 track rows; Tuesday 14 rows (keys 120–128). All hooks mirror `writeTarget` incl. per-type parity quirks (base-vs-routed slots, CurveRate /100, Wavefolder truncate, DiscreteMap snapshot fan-out, Tuesday GateLength clamp no-op). `TestParamRegistry` guards per-table key uniqueness; parity green; sim + STM32 release clean; nothing wired live. **Open owner calls:** Indexed External-sync inlet (DiscreteMapSync borrow), Tuesday Scale/RootNote (dispatched no-ops today → fix-forward not parity), `uint8_t` key width — all U7-ish. Full snapshot in TASK.md.
**Next action:** U4 batch 2 — Stochastic (defect-fixes: Feel dead slot, Scale/RootNote base-write) + PhaseFlux (A/B inlets + nudges + Phase, defect-fix), both intentional non-parity → U5 scaleSource, U6 groups, U6b override+combine (range-class), U7 cutover.
