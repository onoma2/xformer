# Task Board
_Updated: 2026-05-21 (stochastic Phase 11 + follow-ups landed; boredom indicator + short labels + MinDegree/MaxDegree drop)_
...
## 🟡 stochastic-track-port — Phase 11 simplification IMPLEMENTED; hardware re-flash pending
**Status:** paused — Phase 11 refactor done on `feat/stochastic`. Engine + UI collapse complete. Single unified pitch picker (tickets × Complexity kernel × Marbles distro × Steps sieve, all multiplicative). Single unified duration picker (tickets × noteDuration kernel × variation spread). Mode/Level/Character/Shape/Contour/Linearity dropped from engine and UI (reserved fields). Variation sign dropped (symmetric spread). `_marblesSteps` repurposed as the pitch Steps sieve (universal LUT cutoff, parallel to Mask). Duration ticket labels now divisor-aware at draw time. Single flat UI list, semantically grouped. STM32 `.text=882,872` (−600 B vs pre-Phase 11). Sim release builds clean; STM32 release builds clean; all four stochastic test suites pass (TestStochasticGenerator rewritten to assert new multiplicative contract).
**Where I stopped:** Phase 11 implementation + follow-ups landed. Full set: engine drop (contour, linearity, marblesMode branch, durationTicketsActive branch, variation sign), Steps sieve (universal LUT cutoff), Complexity kernel rewrite (linearity baked in), Duration kernel (multiplicative combine), UI flat list, divisor-aware **short** ticket labels via new `ModelUtils::printDivisorShort()` ("x/y" no leading tick value), boredom indicator math synced to engine's Poisson CDF (`StochasticTrackEngine::patienceProbability` helper), MinDegree/MaxDegree dropped silently (reserved as model slots). Test file rewritten for multiplicative contract.
**Next action:** (1) Hardware verification of the unified pickers — confirm musical character (low complexity = soft gravity, high = chaos; low Steps = pentatonic feel, high = chromatic; tickets steer distribution; Marbles bias shifts center; patience indicator climbs and resets at regen). (2) Verify on hardware whether engine actually fires regen at non-100 patience values (user reported 10 min at patience=45 with no regen — likely indicator-only bug from old V5 bucket math, but worth confirming on hardware that the engine path actually fires). (3) Loop-tape pitch direction (off-by-one likely fixed but needs hardware confirmation). (4) Decide whether patience max range should be widened (Proteus parity = max ~43 loops; old V5 = max 128). (5) Naming open question: rename `Steps` → "Tones" or "Sieve" if the universal-LUT-sieve role isn't obvious in field use.
