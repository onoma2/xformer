# Task Board
_Updated: 2026-05-22 (stability-fixes task created from Modulator/Generator adversarial review)_
...
**Where I stopped:** Ownership-contract Patches 1+2 (const_cast removal + intent-named helpers) landed in commit `6309ed8d`. Since then, Batch 0 engine-side completed (lock cache pattern-scoping in `changePattern()`, invalid-PlayMode default `Free→Aligned`, `lockAvailable()` accessor exposed) and page type guards added (`isActiveForSelectedTrack()` early-return on `StochasticSequenceEditPage` draw/updateLeds/keyDown/keyUp/keyPress/encoder; `StochasticConfigPage` and `StochasticPerformancePage` also guarded). UI overhaul: Core + Marbles hero pages removed (−4148 B text), DIRECT renamed LIVE and made default, F5-NEXT cycles Live→Loop→Pitch→Duration→Live. LIVE readout: tiny 1-letter grid (8 cols × 2 rows) + Small full-name held overlay; Duration step shows actual fraction (e.g. `1/16`, `1/8T`) via `printSlotDuration`. Multi-select via step-hold on LIVE + LOOP, Shift toggles persist; multi-hold encoder applies to every selected step; overlay shows `MANY SELECTED` when popcount > 1. Context menu INIT on LIVE + LOOP resets held params (or all bound slots when nothing held). Pitch + Duration ticket-page: ON/OFF badges removed (always-on after Phase 11), bar geometry fixed so Small-font readout no longer overlaps bar tops, always-on tiny Rest % readout on Duration. Pitch/Duration footers got LoopX/LiveX + NewX in the 2 leftmost slots (double-click guard on NewX); existing DROT/MROT/RST shifted right by 2. Renew toasts now full sentences (`Rhythm renewed` / `Press again to renew rhythm`). STM32 `.text=888,624`. Sim + four stochastic test suites pass.
**Next action:** Hardware-verify the full UI overhaul + engine Batch 0. After confirmation, decide whether to (a) tackle the deferred UI items — PlayMode/FillMode hide-or-implement, Lock label, UI gating on `lockAvailable()` — or (b) jump to Batch 1 (Mutate readout `PERM/REGEN/HOLD`, source-state header chip).
...
**Status:** ready — first Modulator/Generator stability backlog captured.
**Where I stopped:** Initial audit found four stability fixes: sanitize `Modulator::read()`, clamp ADSR computed ticks, guard `GeneratorPage` type-specific callbacks, and fix generator relative indexing out-of-bounds.
**Next action:** Implement the four fixes in `Modulator.h`, `ModulatorEngine.h`, `GeneratorPage.cpp`, `SequenceBuilder.h`/`RandomGenerator.cpp`, then run STM32 release build and focused simulator/UI checks.
**Depends on:** nothing
**Branch:** TBD
