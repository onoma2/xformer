# Task Board
_Updated: 2026-06-03 (routing-matrix: sliced cutover plan 005 — slices 1-4 landed TDD+Codex-clean. The cutover is COMPLETE: Note+PhaseFlux per-track routing now runs the override path end-to-end. Slice 4 migrated all 22 getters to clamp(base+override) + gated every interactive knob-edit path on override presence; PhaseFlux Scale/RootNote base-write defect fixed. Live Note/PhaseFlux routing restored (was inert mid-slice). Next: slice 5 = hardware audition.)_
...
**Next action:** Slice 5 — hardware audition: Note + PhaseFlux, Modulate, None/TriangleFold. Expect forward-model sound (base-anchored, bipolar around base), NOT like-for-like with old absolute routes (audition caveat). Then slice 6 (UI: persisted combine/d/scaleSource) → 7 (other six getters) → 8 (U7 serialized format) → 9 (delete old writeTarget/Routable routed half).
