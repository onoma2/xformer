...
**Where I stopped:** PitchMode + pitchRate just wired. `PhaseFluxSequence::pitchMode = Cell | Global` (sequence-owned, not track-owned — different sequences on the same track can run different modes), `_pitchRate` 1-byte ratio index (17-entry P:T table, 1:1 default), `_pitchPhase` free-running accumulator advancing every tick. Global mode: `stage[0]`'s pitch curve/warp/response/flips ARE the globals (no new per-stage storage); engine reads them at `_pitchPhase` instead of `_stagePhase`. basePitch / range / accumulator stay per-cell. Roundtrip lossless. Hero edit PTCH set now mode-aware: header `PTCH` / `PTCH.G`, stage badge `n` / `G`, footer Curve/Warp/Resp/Rate/Note in Global (F1-F3 edit stage[0], F4 sequence pitchRate, F5 displays active stage's note name + range dots stacked). Shift+F5 in Global cycles range (demoted); F5 label swaps `NOTE` → `SPAN` while Shift held. Footer `Rng` renamed `Span` everywhere. PitchMode + PitchRate rows added to PhaseFluxSequenceListModel. Spec updated: §6.2.3 + §12.2 + §17.2.1 + §17.3. UI-preview variants at `ui-preview/phaseflux-pitchmode/`.
**Next action:** Flash to hardware. Smoke test Global mode: switch a sequence to Global in the sequence list, twist Rate ratio, verify cell revisits sample different pitch positions. Then audit open §14.1 questions (Q8..Q12) and decide on hysteresis for Always mode.
**Depends on:** nothing
