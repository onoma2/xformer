# Task Board
_Updated: 2026-05-22 (Stochastic Lock engine path removed — heap-stack collision crash on >=2 tracks fixed; Lock redesign deferred to LOCK-DESIGN-DEFERRED.md, possibly absorbed by fractal trunk)_
...
**Where I stopped:** Ownership-contract Patches 1+2 and the UI overhaul (Live/Loop/Pitch/Duration four-page model, multi-select, full-sentence toasts) shipped in commits `6309ed8d` and `a2208039`. Then a fresh project with two stochastic tracks was found to hardfault on reboot — root cause was the `StochasticTrackEngine` heap allocation (`new LockedParentEvent[64]` = 5,888 B per track) blowing through the ~12 KB combined heap+stack window on STM32. PROJECT.md gained a C++ Heap section documenting this constraint. The lock-cache heap path was **deleted** 2026-05-22: `_lockedParents`, `LockedParentEvent`/`LockedChild`, `initLockedSteps`/`freeLockedSteps`, `captureLockedParent`, `lockAvailable()`, the destructor, and the `if (locked)` replay branch in `triggerStep` all removed. `.text` dropped to 886,076 (−2,548 B). The `_lock` flag on `StochasticTrack` remains as a UI placeholder (the Config-page toggle still exists and is settable) but the engine ignores it. Lock redesign is captured in `.tasks/stochastic-track-port/LOCK-DESIGN-DEFERRED.md` — flat tick-event tape at 768 B/track in CCMRAM, with an open question about absorbing Lock into the fractal-track trunk substrate post-implementation. Sim + STM32 release builds clean. Four stochastic test suites pass.
**Next action:** Hardware-verify the heap-crash fix with a fresh project + two stochastic tracks + reboot. After confirmation, the remaining stochastic UI items (PlayMode/FillMode hide-or-implement, Mutate `PERM/REGEN/HOLD` readout, source-state header chip) move to the next session. Lock redesign waits on fractal-track implementation.
