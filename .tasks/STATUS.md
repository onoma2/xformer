# Task Board
_Updated: 2026-05-22 (stochastic Phase 12 round 2: Codex adversarial fixes — mutation pipeline, Mask/Tilt window scoping, law fidelity, musicality cliffs)_
...
## 🟡 stochastic-track-port — Phase 12 round 2 landed; hardware re-flash pending
**Status:** paused — Phase 11 + Phase 12 round 1 (Codex-flagged bugs) + Phase 12 round 2 (musicality cliffs) all on `feat/stochastic`. Five Codex adversarial reviews ran and all actionable findings were applied. STM32 `.text=884,996` (+556 B vs Phase 12 baseline for the additional fixes). Sim + STM32 release build clean; all four stochastic test suites pass.
**Where I stopped:** Round-2 fixes applied: resetMeasure preemption guard (no double-counting); mutation targets `[first, last]` (was full `size` — likely cause of "negative mutate doesn't work" when window narrowed); `rollPatience` runs before mutate so domains about to regen don't get wasted writes; Mask/Tilt ranks window-local (rank only `[first..last]`, mask cutoff against `windowSize`, re-rank on First/Last edit too); Burst divisor mismatch fixed (parentTicks computed from actual divisor not legacy 1/16); First/Last window leak fixed (snap `_patternIndex` at triggerStep entry); Steps=0 serialization clamp; SHAPE toggle removed (was a lie — engine always honors Bias/Spread/Steps); Contour drift cap to kernelWidth; Patience smoothed (dropped knob-100 off-sentinel, λ ∈ [1, 80] linear); Steps shows K/N on Marbles hero; Burst dead-zone marker `*` in `printBurst`. Then integer-truncation cliff fix on five knobs: Variation, Complexity, Marbles Spread, Drift cap scale, Sleep rounding. Same fix shape: ×10 scaling of the kernel triangle term so center keeps strong lead vs leakage floor — at VAR=10, P(center) goes from old 22% cliff to new 61% smooth. Also rendered Python preview of two ticket-overlay designs (strip-above + bell-as-background) in `ui-preview/render_ticket_overlay.py` with 14 PNG variants — bell-as-background with trimmed bars is the chosen direction for porting to C++.
**Next action:** (1) Commit Phase 12 round 2 (this batch). (2) Hardware verification of the corrected mutation pipeline (Mask/Tilt window-local, mutate-before-patience ordering, smooth Variation/Complexity at low knob values, patience no longer cliff at 100). (3) Port ticket overlay (bell-as-background with trimmed bars) from Python preview to C++ `drawPitchPage`/`drawDurationPage`. (4) Loop-tape pitch direction confirmation. (5) Decide patience max range (currently λ ∈ [1, 80] — Proteus parity was 1..50). (6) Naming open: rename `Steps` → "Tones" or "Sieve" if the sieve role isn't obvious in field use.
**Branch:** feat/stochastic
**Reference:** `.tasks/stochastic-track-port/PHASE11-SIMPLIFY.md` (active spec — drafted 2026-05-21), `.tasks/stochastic-track-port/PHASE7-DICTIONARY.md` (canonical, updated 2026-05-21), `.tasks/stochastic-track-port/PHASE10.7-L1-REDESIGN.md`, `.tasks/stochastic-track-port/PHASE10.6-BUG-CATALOG.md`, `.tasks/stochastic-track-port/UI-CONTROL-DRAFT.md`, `ui-preview/pages_levels.py`, `ui-preview/render_levels.py`, `ui-preview/level-previews/`
