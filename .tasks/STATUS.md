...
**Status:** Accumulator v2 + ACCUM hero redesign + TEMP/PITCH scope visuals + 4+Next paged footer across all topics + **Repeat & Window per-axis curve transforms wired end-to-end** + **pulseCount range expanded to 0..16 with no ProjectVersion bump** (split-bit encoding: low 3 bits stay in `_data0`, high bit + mute sentinel in `_data3` spare; legacy files decode unchanged). `pulseCount=0` = silent stage (duration consumed, no events fired); `pulseCount=16` = max density. Engine clamp `[0, kMaxPulses=16]`; `kMaxPulses` bumped 8→16. `pulseAccumConfig` default limits 8/8 → 16/16. UI clamps and randomize range updated; TEMP scope verified visually fine at 16-pulse max density via `ui-preview/tempo-scope/`.
**Where I stopped:** ACCUM hero redesign landed: 4×4 grid + dual-strip + 5-row param list replaced with a 1×16 cell row at y=28..36, per-cell bipolar `Ac.St` pancakes (right half of each cell, magnitude UI-clamped to 7), per-cell S/P trig chips (left half, below square), three hand-drawn 5×5 glyphs in the left margin for Order / Polar / Reset. Footer paged: P0 `Ac.St / — / — / Order / Next`, P1 `Reset / Polar / Trig / — / Next`; F5 cycles `_accumPage`. `Amount` renamed `Ac.St` everywhere. `+Lim` / `-Lim` removed from the hero page — `PhaseFluxSequence::clear()` defaults to max span (note 28/28, pulse 8/8) so the accumulator is unbounded out of the box; limit edits will land on the sequence list page in a follow-up. Scope binding dropped from UI (data field preserved). Shift bindings dropped for ACCUM. Track 8 default-project demo simplified to a clean testbed: 4 active stages, 12 skipped, all `pulseCount=1` + `pitchCurve=Ramp`, no demo-curve variations, no accumulator overrides. Memory rule added: `feedback_data_scope_vs_visual_repetition.md` (don't paint N identical copies of a sequence-wide value). Earlier work intact: AccumulatorConfig + AccumulatorOps + engine counter advance + cycle wrap + auto-reset for both axes; post-audit bug fixes (changePattern measure-clear, pulse defaults, 1-stage silence, Pendulum signed-step, three adversarial-review math bugs); engine 480 B, Track 4772 B; static_assert ceiling at 900. Repeat / Window curve transforms drafted in spec §6.1.2/§6.1.3/§6.2.4/§6.2.5 + §14.2 — not yet wired.
**Next action:** Hardware audition of the redesigned ACCUM/TEMP/PITCH hero pages — verify the new paged footer (L/R = topic, F5 = Next, FlipV/FlipH press-to-flip on Page 1), the pitch-scope label column + top-4-degree readout + 2×2 pulse-fire dots, and the accumulator v2 Note/Pulse drift behaviors against §13.10 Task 10 acceptance gates. After audition: wire `+Lim` / `-Lim` editing on `PhaseFluxListPage` (deferred); decide whether to wire Repeat + Window. Open questions still in §14.1 Q9..Q12 (cycleSpread, global phase warp, snap-to-grid utility, hysteresis for Always mode).
**Depends on:** nothing
