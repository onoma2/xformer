# Task Board
_Updated: 2026-05-21 (engine-rebuild-order crash fix on init project)_
...
## 🟡 stochastic-track-port — Phase 2 + init-project crash fix landed; hardware verification next
**Status:** paused — Phase 2 deterministic-cluster work complete (7 commits, all 13 catalogued bugs fixed). Init-project hard-reboot regression fixed by moving Engine `updateTrackSetups()` before clock-event processing. Hardware-tuning steps (scale-aware Marbles, bipolar Mutate, engine-clever derivation) explicitly deferred per plan.
**Where I stopped:** Seven cluster commits between `01752a25` (plan) and `ef0ab4cb` (last cluster D): Cluster A serialization (`_density`/`_durationTickets`/`_level` added to stream, `_lock` dropped per PHASE9-V4), Duration dictionary (1/4..1/64 + triplets + 3/16), L1 fan-out drops (editDensityMacro / editComplexityMacro single-field), 3-way pitch exclusivity (tickets > Marbles > Complexity), Cleanup (8 dead Track fields + `_reservedJitter` + debug printf + duplicate reset zeros), Cluster B (resetMeasure/restart counter clears + variation gating + lock-branch _eventDuration write), Cluster D (parent gate-OFF gated on `!hasChildren` in both locked and live branches + 6-tick child gate floor). Plus Engine.cpp engine-rebuild-order fix: `updateTrackSetups()` now runs before clock-event loop in `update()`, eliminating the stale-trackEngine-pointer hard fault that surfaced on init project when the model union was swapped to NoteTrack but the engine container still held StochasticTrackEngine. Hardware confirmed: init project no longer reboots. Four unit tests passing in sim; pre-existing Tuesday/Curve test failures unrelated.
