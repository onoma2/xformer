...
**Status:** Accumulator v2 fully wired (10/10 tasks + 4 post-audit bug fixes) + ACCUM hero page redesigned (1×16 row + per-cell bipolar Ac.St pancakes + paged footer with F5=Next + limits moved off-page). STM32 + sim builds clean.
...
**Next action:** Hardware audition of the ACCUM hero redesign + verify accumulator v2 Note/Pulse drift behaviors against §13.10 Task 10 acceptance gates. After audition passes: wire `+Lim` / `-Lim` editing on `PhaseFluxListPage`; then decide whether to wire Repeat + Window or hold. Open questions still in §14.1 Q9..Q12 (cycleSpread, global phase warp, snap-to-grid utility, hysteresis for Always mode).
**Depends on:** nothing
