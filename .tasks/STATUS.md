# Task Board
_Updated: 2026-05-21 (stochastic Phase 12: gate length, drift, repeat, patience split, perform pair Mask/Tilt, loop hero rewire)_
...
## 🟡 stochastic-track-port — Phase 11 + Phase 12 landed; hardware re-flash pending
**Status:** paused — Phase 11 (engine collapse + unified pickers + flat list + divisor-aware labels + MinDegree/MaxDegree drop + accent gate output drop) AND Phase 12 (gate length, drift, repeat, patience split, Mask/Tilt performance pair, loop hero rewire, double-click guard) all implemented on `feat/stochastic`. STM32 `.text=884,440` (+968 B vs Phase 11 baseline for all Phase 12 features). Sim + STM32 release build clean; all four stochastic test suites pass.
**Where I stopped:** Phase 12 fully implemented in one session. Reserved-slot table fully populated: `_density` → Gate Length spread (via `pickGateLength` with 10% + 6-tick floor); `_contour` → Drift directional bias (re-enabled in Complexity kernel); `_linearity` → Repeat probability (Bernoulli at triggerStep reuses cached `_lastEvent`); `_accentProb` → Patience M (split per-domain Poisson, own counter); `_marblesSteps` → pitch Steps sieve. Mask + Tilt now a Loop-mode filter+resonance pair: Mask = cutoff 0..100, Tilt = bipolar duration-aware rank weight (long ↔ short). Engine caches `_lastAppliedTilt`/`_lastAppliedSize` for instant re-rank on knob edit. Mutate-rhythm also triggers re-rank. Loop hero step buttons rewired (top 0..4 patience R/M/mutate/jump/sleep, bottom 8..11 window, 14..15 mask/tilt) with red/green LEDs. Boredom bars split per domain growing rightward, 2px gap, scaled to new `patienceMeter` (linear loops/λ) instead of Poisson CDF (which never visibly filled). Double-click guard on F3 NewR / F4 NewM via `event.count() == 2`. Accent gate output silently dropped (no second jack on device).
**Next action:** (1) Commit Phase 12. (2) Hardware verification — confirm Mask+Tilt feels musical (long-note vs short-note survival), boredom bars visibly fill before regen, patience split allows independent R/M renew rates, double-click guard prevents accidental NewR/NewM, gate length spread is audible at high values. (3) Loop-tape pitch direction (off-by-one likely fixed; needs hardware confirmation). (4) Decide whether patience max range should be widened (Proteus parity = max ~43 loops; old V5 = max 128). (5) Naming open: rename `Steps` → "Tones" or "Sieve" if the universal-LUT-sieve role isn't obvious in field use.
