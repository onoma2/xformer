# Task Board
_Updated: 2026-05-25 (Stochastic, third session pass — performance gesture cleanup + Live page reshuffle: Live capture moved to NewR/NewM (mode-aware, auto-switches to Loop), mode-toggle buttons reverted to pure toggle; cache walk now reads events[] when `_capturedFromLive*` flag is set so the captured shadow actually plays; Pitch + Duration quick-edit slots aligned with Live page at visual steps 13/14/15 (Init/Even/Random); Live page step layout reshuffled for semantic grouping — rhythm/articulation top-row, burst in top-right + bottom-right corners, pitch shape bottom-row, Repeat standalone; finalize.md punch list captured as the closing gate before this task can be marked done. Previous-session highlights (still landed): Mutate unipolar-destructive; Duration/Variation/Rest three-zone law + independent rest dice; Burst 4-state pitch×timing + Fit-curve trap flams; childCount→burstTails + BurstCount=total cells; Patience log2 curve; cluster floor 12 ticks; Tier 4 ContentSnapshot; Shift+NewR/NewM undo; PAGE+step15 Live randomize; pitch generator 5-step sequential law; HTML knob inventory; LOCK-DESIGN-DEFERRED with A/B candidate.)_
...
## 🟡 stochastic-track-port — Pitch + Rhythm law rewrites + performance gestures + Live page reshuffle landed
