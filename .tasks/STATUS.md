...
**Spec 013 implementation — phases 1-4 + 5a DONE.** Followable board: `.tasks/routing-matrix/SPEC-013-TODO.md`. Owner cadence: build to phase 7, gate each by **STM32 release build + Codex** (no per-phase flash); owner flashes ONCE after phase 7. **FLASH is now the hard constraint** (960 KB app partition, was at the ceiling): reclaimed via bias-overlay removal (~5 KB), Asteroids gate `CONFIG_ENABLE_ASTEROIDS` off (~14.5 KB), and 5a legacy-editor removal (~9.7 KB) → comfortable headroom. Flash culprits + measurement in PROJECT.md "Flash ceiling snapshot".
...
- **Phase 5 (matrix door) — IN PROGRESS.** Entry settled (spec §15): the **ROUTING menu page IS the matrix**. **5a DONE (`89cf0fe5`):** removed legacy overview/per-route editor/RouteListModel; ROUTING opens straight into the tab editor (matrix precursor). **5b = NOT STARTED (next):** rebuild the tab editor's single-column list into the spec-§5 8-col numeric grid (number+engine headers `1N 2C 3N…`, `-`/`.`/number cells, cursor=brightness not a box, band header + DEPTH/SOURCE + MODULATE, unify track-letter helper). Then 5c per-row draft/commit cell-depth edit, 5d T-toggle+by-type, 5e F1 VIEW depth↔source, 5f REMOVE. Renders `ui-preview/modulation/mod-matrix*.png`.
- **Deferred:** phase 6 per-engine migration (Curve/Tuesday/Stochastic/DiscreteMap/Indexed live), phase 7 MIDI LEARN (F4→F1) + shaper, phase 9 delete old writeTarget/Routable half.
