# Task Board
_Updated: 2026-06-03 (routing-matrix: engine units started — `RouteApply.h` value pipeline (scaleSource + combine → delta) landed as a pure TDD unit; re-anchor resolved as Align/lifecycle (subspec 004), DiscreteMap Sync inlet dropped. Next: override table + shaper stage.)_
...
**Next action:** Override table (transient `(track,paramKey)→delta`) → shaper stage feeding RouteApply → slot model → U6 groups → U7 re-address + engine cutover.
