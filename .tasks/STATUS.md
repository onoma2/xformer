...
**Next action:** Slice 6 **model half DONE** — Global base+delta (997f8b84) + combine/scaleSource persisted on Route (d8a4a16b; `d` = existing `_depthPct`, clear/==/serialize, appended unconditionally per post-V35 policy, no bump). Note/PhaseFlux+Global all base-anchored; engine still hardcodes Modulate in computeDelta (no behavior change until the editor sets combine). **Now: the UI surfaces** per spec 009 — scope lens, vertical-tab editor, spread, target lens, track+engine mini-map; first pass exposes Note+PhaseFlux+Global only. Then 7 (other six engines + their UI tabs, paired) → 8 (U7 serialized format) → 9 (delete old writeTarget/Routable routed half). **Carry-forward:** Python `Project.tempo`/`swing` bindings still getter+base-setter (latent read-modify-write absorb; no live caller) — add base-getter binding when the Python surface is next touched.
