...
**Where I stopped:** On `refactor/routing-matrix`: U1 `RouteParam` (+fail-closed guard), trigger unification. **Shared `ParamKey` registry** (`model/RouteParamKey.h`, append-only authority, F6); **U2 Global + U3 Note + U4 Curve + Indexed + DiscreteMap tables on it.** Curve validates "one key, many types"; **Indexed established the first-class INLET row-kind** (A/B); **DiscreteMap owns Input/Scanner/Sync inlets** (track-level, raw float) + resolved SlewTime≡SlideTime (→ seq slewTime). All hooks mirror `writeTarget` incl. parity quirks (CurveRate /100, Wavefolder/DjFilter truncate→int16_t; Indexed/DiscreteMap base-writes for Divisor/Scale/RootNote/Octave/etc; routed slot only for SlideTime/ClockMult). `TestParamRegistry` guards per-table key uniqueness; parity green; sim + STM32 release clean; nothing wired live. **Param triage RESOLVED.** Open owner calls: Indexed External-sync inlet (DiscreteMapSync borrow) + `uint8_t` key width — both U7. Full snapshot in TASK.md.
**Next action:** Remaining U4 tables: Stochastic (defect-fixes: Feel dead slot, Scale/RootNote base-write), PhaseFlux (A/B inlets + nudges + Phase, defect-fix), Tuesday, MidiCv → U5 scaleSource, U6 groups, U6b override+combine (range-class), U7 cutover.
