# Task Board
_Updated: 2026-05-21 (stochastic engine + mutate LUT bias landed; sim-verified, hardware re-flash pending)_
...
## 🟡 stochastic-track-port — Engine + mutate-LUT overhaul landed; hardware re-flash pending
**Status:** paused — major engine + UI reshape complete on `feat/stochastic`. Hero pages (CORE / MARBLES / DIRECT / LOOP) live. `rate → noteDuration` rename with divisor-relative LUT. Variation Approach 3. Bipolar mutate with universal-LUT bias on the destructive side. Cluster F burst gate + burst count/spacing LUTs. Engine event-driven step advancement. Live-mode write-back. Per-domain renew. Loop-page F-keys redesigned (LoopR/LoopM/NewR/NewM). Loop tape divisor-aware-and-adjusted with centered layout. Sim release builds clean; STM32 release builds clean; all four stochastic test suites pass.
**Where I stopped:** Mutate-LUT bias commits (`42a2475b` major overhaul, `3a2637a9` wiki, plus uncommitted in-progress work for universal pitch LUT + rhythm anchor LUT). Mutate magnitude now couples probability with bias strength: low magnitude = rare + tonal-anchored, high magnitude = frequent + uniform random. Pitch path uses `universalDegreeBoost(degInOct, N)` with anchors at integer fractions of N (root, N/2, N/3, 2N/3, N/4, 3N/4) — works for any scale size including microtonal. Rhythm path uses triangular kernel around current `noteDuration` slot with additive `variation` direction bias. Jump rewritten as Proteus-style bounded random walk (±2 octaves, non-destructive playback offset). STM32 `.text=889,360`, SRAM unchanged at 90.7%.
**Next action:** (1) Commit pending mutate-LUT bias + jump walk + dictionary updates. (2) Hardware verification of the entire stochastic stack on the new event-driven engine — confirm timing matches expectation across noteDuration slots, divisor changes scale correctly, variation produces audible per-event durationIndex spread, mutate magnitude knob shape (anchor → chaos) lands musically. (3) Loop-tape pitch direction (tallest bar reportedly plays lowest pitch — likely off-by-one fixed but needs hardware confirmation). (4) UX call on dual Character/Complexity list entries. (5) Consider preset ticket configurations (Major Triad / Pentatonic Bias / Tonic-Dominant) as quick-load buttons on the L3 ticket page — provides Proteus-like musical defaults for users who don't customize. Not urgent.
**Branch:** feat/stochastic
**Reference:** `.tasks/stochastic-track-port/PHASE7-DICTIONARY.md` (canonical, updated 2026-05-21), `.tasks/stochastic-track-port/PHASE10.7-L1-REDESIGN.md`, `.tasks/stochastic-track-port/PHASE10.6-BUG-CATALOG.md`, `.tasks/stochastic-track-port/UI-CONTROL-DRAFT.md`, `ui-preview/pages_levels.py`, `ui-preview/render_levels.py`, `ui-preview/level-previews/`
**Branch:** feat/stochastic
**Reference:** `.tasks/stochastic-track-port/PHASE10.7-L1-REDESIGN.md`, `.tasks/stochastic-track-port/PHASE10.6-BUG-CATALOG.md`, `.tasks/stochastic-track-port/PHASE10-V5-CONTROL-GRANULARITY.md`, `.tasks/stochastic-track-port/PHASE7-DICTIONARY.md`, `.tasks/stochastic-track-port/PHASE9-V4-OWNERSHIP.md`, `ui-preview/pages_levels.py`, `ui-preview/render_levels.py`, `ui-preview/level-previews/`
