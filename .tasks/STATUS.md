# Task Board
_Updated: 2026-05-22 (stochastic DIRECT hero page: event-driven walker, full 16-slot mapping incl. range/gate/slide/legato, variation default 16)_
...
## 🟡 stochastic-track-port — DIRECT hero page complete; hardware re-flash pending
**Status:** paused — Phase 11 + Phase 12 r1 (Codex bug pass) + Phase 12 r2 (musicality cliffs) + DIRECT hero redesign all on `feat/stochastic`. STM32 `.text=887,212` (+2,216 B vs r2 baseline, the DIRECT walker + 4 new step-button mappings). Sim + STM32 release build clean; all four stochastic test suites pass.
**Where I stopped:** DIRECT hero is now the full pitch+rhythm performance page: 16 step buttons mapped — top row red 0=DURA/1=VARI/2=REST/3=RANG, top row orange 4=BURS/5=BCNT/6=BRAT/7=BPIT, bottom row green 8=CMPX/9=CONT/10=BIAS/11=SPRE/12=REPT, bottom row red 13=GATE/14=SLID/15=LEGA. Centered "walker" visualization: event-driven ring of 12 particles, fed by gate rising edges on the live engine; horizontal = rhythm (events arriving), vertical = pitch (derived from `cvOutput(0)`); midpoint between walker head and oldest tail particle anchored at viewport mid-X; trail brightness floor bumped so all 12 slots are visible. Top tiny-font macro readout spans DUR/VAR/RES + BUR/BCN/BRT/BPI (row 1) and CMP/CON/BIA/SPR/REP (row 2); held-step shows full label including all four new params. Bell-haze background unchanged. Footer mirrors LOOP page — Loop/Live + NewR/NewM + NEXT. Also: investigated "newR does nothing at defaults" — root cause was rhythm-side defaults all deterministic (variation=0, rest=0, burst=0, legato=0, slide=0, accent=0 → identical buffer per renew); fixed by bumping `_variation` default from 0 to 16 in both `clear()` and `sanitizeAfterRead()` so newR produces audible duration drift out of the box.
**Next action:** (1) Commit DIRECT hero work (this batch). (2) Hardware verification of the corrected mutation pipeline (r2 fixes) + DIRECT page event-driven walker + new range/gate/slide/legato mappings. (3) Port ticket overlay (bell-as-background with trimmed bars) from Python preview to C++ `drawPitchPage`/`drawDurationPage`. (4) Loop-tape pitch direction confirmation. (5) Decide patience max range (currently λ ∈ [1, 80] — Proteus parity was 1..50). (6) Naming open: rename `Steps` → "Tones" or "Sieve" if the sieve role isn't obvious in field use.
