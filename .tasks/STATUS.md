# Task Board
_Updated: 2026-05-23 (Phase 16 P1-P5 landed: queue overflow safety, cell packing, kCellCap=80, Feel knob inert, Feel scaling integrated. User-visible Feel knob ready to flash. Flat-cell refactor + P6-P8 (CycleEndPlan, mutation log, Repeat baked, events[] deletion) deferred to next session)_
...
## 🟡 stochastic-track-port — Phase 14B landed; Phase 16 flat-cell drafted
**Status:** Phase 14B Patches A/B/C step 1/1b/2 shipped on `feat/stochastic-seed-log` and hardware-verified through 8-track save/load. STM32 release: `.text=890,684`, `.data=6,416`, `.bss=168,004`, `.ccmram_bss=55,084`. All 6 stochastic suites green (TestStochasticCacheParity has 20 cases). Patch C step 3 / Repeat-bake / mutation-log / events[] deletion are now folded into **Phase 16 — flat cell model + Feel knob** (drafted 2026-05-23, replaces parent/child concept entirely).
**Where I stopped:** Phase 14B sequenced commits: keyed-RNG (`9f0fb842`), Patch A (`168292c2`), Patch B (`f1551e1b`), Codex fixes (`41f858fd`, `e5c1868d`), cache-as-engine-member (`eceb9bf0`), patience=100 off-sentinel (`93485dc7`), window-edit sync (`831d3553`), C step 1 pitch (`5efe377b`), C step 2 parent fields (`f8d892e4`), C step 1b burst-child notes (`a3bab12d`), Codex review #2 fixes (window-sync from perf page, burst-shaping cache refresh, gate fraction encoding `fef7c712`), LOOP highlight window-aware (`26f5920f`). Non-Repeat path entirely cache-driven; Repeat path still events-tape for legacy compatibility. Then a research pass on what's actually needed for odd-time / polyrhythm / flat identity led to drafting **Phase 16**: ditch parent/child entirely, replace with interval-walk flat cell model + one new knob ("Feel") that controls cycle alignment to `resetMeasure` bar. Burst becomes a duration-bias on the picker, not a child-spawn. Per AGENTS.md line 81, no version bump / migration ceremony — dev projects disposable.
**Next action:** Hardware-flash and verify Feel knob (P5). Audibly:
- Default Feel=50 = behavior identical to today (detent, no scaling).
- Feel<45 compresses cycle toward 3 beats; Feel>55 stretches toward 5.
- Edit Feel routes via `Routing::Target::StochasticFeel` (ID 67).
Then return for **flat-cell refactor + P6/P7/P8** as a separate session: rewrite generator as interval walk with burst-as-duration-bias (eliminates parent/child cache structure, drops `evaluateChildren`, drops burstChild aux flag, simplifies triggerStep ~80 LOC), then CycleEndPlan struct (P6), mutation log + Repeat baked + seed-lock guard (P7), and finally delete `_events[]` + `StochasticSourceEvent` (P8) per AGENTS.md disposable dev-projects rule.
**Branch:** feat/stochastic (stable Phase 13); feat/stochastic-seed-log (Phase 14B + 16 work)
**Reference:** `.tasks/stochastic-track-port/PHASE16-FLAT-CELL.md` (drafted 2026-05-23, **current direction**), `.tasks/stochastic-track-port/PHASE14B-SEED-LOG.md` (drafted 2026-05-22, foundation that landed), `.tasks/stochastic-track-port/PHASE15-PITCH-MATH-REVIEW.md` (drafted 2026-05-22, queued after Phase 16), `.tasks/stochastic-track-port/PHASE14-TRUNK-MODEL.md` (considered alternative — retained for reference), `.tasks/stochastic-track-port/LOCK-DESIGN-DEFERRED.md` (folded into Phase 14B as content-lock, deferred to future), `.tasks/stochastic-track-port/PHASE13-FEATURES-PLAN.md` (cleanup history), `docs/stoch-review.md` (adversarial review), `.tasks/stochastic-track-port/PHASE11-SIMPLIFY.md`, `.tasks/stochastic-track-port/PHASE7-DICTIONARY.md`, `.tasks/stochastic-track-port/PHASE10.7-L1-REDESIGN.md`, `.tasks/stochastic-track-port/PHASE10.6-BUG-CATALOG.md`, `.tasks/stochastic-track-port/UI-CONTROL-DRAFT.md`
