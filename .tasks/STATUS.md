...
**Where I stopped:** On `refactor/routing-matrix`: U1 `RouteParam` (+fail-closed guard), trigger unification. **Shared `ParamKey` registry** (`model/RouteParamKey.h`, append-only authority, F6); **U2 Global + U3 Note + U4 Curve tables on it.** Curve validates "one key, many types" (reuses 8 keys shared with Note; appends Offset/ShapeBias/Wavefolder/DjFilter/Chaos×4/CurveRate + Phase Tier-2b). `TestParamRegistry` guards per-table key uniqueness; writeTarget-parity green (Curve hooks mirror exactly: CurveRate /100, Wavefolder/DjFilter truncate→int16_t; Phase is launch-only, tested direct). Sim + STM32 release clean; nothing wired live. **Param triage RESOLVED:** launch = routable-today + Phase + PhaseFlux A/B inlets + five nudges + two defect-fixes; Teletype out; SlewTime≡SlideTime; rest deferred. Codex U7 watch-item: `uint8_t` key width settled at cutover (~130–160 keys fit). Full snapshot in TASK.md.
**Next action:** Remaining U4 per-type tables (Tuesday, DiscreteMap, Stochastic, PhaseFlux, MidiCv, Indexed — parity port + appended keys + sweep; Stoch/PF carry defect-fixes; several introduce **inlets**, a first-class target kind) → U5 scaleSource, U6 groups, U6b override+combine (range-class), U7 cutover.
