...
**Where I stopped:** On `refactor/routing-matrix`: U1 `RouteParam` (+fail-closed guard), U2 `GlobalParamTable`, trigger unification (Reset/Play/Record/TapTempo → one `RouteState.gateMask`), U3 first type `NoteParamTable` — all parity-tested, green, nothing wired live. U2/U3 per-type tables predate the registry+`d` model and get reworked. **Param triage RESOLVED:** launch = routable-today + Phase (shared) + PhaseFlux A/B inlets + all five PhaseFlux nudges + the two defect-fixes (StochasticFeel, Stoch/PF Scale-RootNote base-write); Teletype out of the matrix; SlewTime≡SlideTime; all other deep params deferred (append per-key when used). Full snapshot in TASK.md.
**Next action:** Build the registry (grouped concepts, range-class, per-type apply bindings) from the resolved launch set → rework U2/U3 onto it → U4 inlets, U5 scaleSource, U6 groups, U6b override+combine, U7 cutover.
