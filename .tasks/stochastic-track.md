...
## Decisions log
- 2026-05-21 (Hero-page UI control mapping + responsive renders): After hero-page picks (CORE-RAIN / MARBLES-BELL / DIRECT-POND / LOOP-TAPE), audited five existing edit pages — `TuesdayEditPage`, `IndexedSequenceEditPage`, `DiscreteMapSequencePage`, `GeneratorPage`, `TeletypeEditPage` — to extract the canonical Performer input vocabulary and the recurring page patterns. Added §12 "UI control vocabulary" to `docs/performer-architecture.md`: 12.1 input primitives table (Step / Function / Shift / Page / Page+Step / Page+Shift / double-press / Left+Right / Encoder turn + press), 12.2 the six common patterns (A = params console à la Tuesday, B = step-tab + sub-page, C = multi-select with shift-double = "match all" à la DiscreteMap, D = generator preview workflow with A/B + re-roll, E = view-only delegate à la Teletype, F = live status box widget), 12.3 LED bi-color conventions, 12.4 reusable widgets (ContextMenu, StepSelection, QuickEdit modal, showMessage), 12.5 "what not to invent" rules. Stochastic-specific control map drafted at `.tasks/stochastic-track-port/UI-CONTROL-DRAFT.md` — per-page step/F-key/shift/page mapping using only real `StochasticSequence.h` / `StochasticTrack.h` field names: CORE step1/2 = density/complexity, MARBLES step1/2/3 = marblesBias/Spread/Steps (Pattern A), DIRECT step1-8 = variation/tilt/contour/rate/linearity/rest/slide/jump (raw L2 — corrected earlier wrong "monitor-only" framing; Direct level means raw L2 params per `StochasticLevel::Direct`), LOOP step1-6 = mutate/patience/sleep/first/last/rotate plus read-only boredom/sleep-countdown/jump-register viz. Shift+F4 from any hero opens list-model fallback for non-hero params. Encoder edits the held step (default = first step of page when none held). Page+Shift = context menu, Page+Step8-15 = QuickEdit slots. LED color domains: orange = Core macros, green = Marbles/Direct, red = Looper, both = held/active. Drawn UI elements are reactive: `render_marbles_bell()` in `pages_levels.py` now takes optional bias/spread/steps/mode overrides; 12 sweep variants rendered (`marbles-bell-bias-{20,50,80}.png`, `-spread-{20,50,90}.png`, `-steps-{2,8,16}.png`, `-mode-{off,soft,hard}.png`) showing peak shift / curve width / quantization density / curve sharpness response — visibly readable, not engine-exact. Driver: `ui-preview/render_marbles_response.py`. Pending: same reactive treatment for tide/rain/pond/tape; add boredom/sleep readouts to loop-tape; add page-nav dot strip to header.
- 2026-05-21 (Hero-page UI draft — norns-style, level-as-image): Started a poetic-image UI direction for the four Stochastic levels (Core, Marbles, Direct, Looper). One hero image per level, sparse text, control via single-step-per-param + Fn keys for toggles/actions. Eleven preview variants rendered at pixel-exact 256×64 in `ui-preview/level-previews/`. User picked: `core-rain` (Density / Complexity / Shape macros as a falling-dot field), `marbles-bell` (smooth distribution silhouette), `direct-pond` (live CV+gate as ripples on water), `loop-tape` (horizontal tape with window bracket + mutation scratches). Renderers live in `ui-preview/pages_levels.py`; driver in `ui-preview/render_levels.py`. Control mapping grounded in the real list model (`StochasticSequenceListModel.h`, `StochasticSequence.h`) — one continuous param per step button (encoder writes when held), Fn keys for on/off toggles and actions, read-only engine state (boredom = `_loopCycleCount`, sleep countdown, jump register) visualized only. Confirmed mapping per page: CORE-RAIN steps 1-2 → `density`/`complexity`, F1 MODE (rhythm+melody Loop/Live), F2 SHAPE (`marblesMode` toggle, gates Marbles page), F3 RENEW (`refreshLoopSources()`), F5 NEXT. MARBLES-BELL steps 1-3 → `marblesBias`/`marblesSpread`/`marblesSteps`, F1 SHAPE back to Core. DIRECT-POND has no step controls — monitor only. LOOP-TAPE steps 1-4 → `mutate`/`patience`/`sleep`/`jump`, steps 5-6 → `first`/`last`, F1 MODE, F2 RENEW. LED color domains: orange = Core macros, green = Marbles family, red = Looper/Mutation family, dim = monitor. OLED can't render color so we mirror with pattern (filled/outlined/half-fill). Pending: implement Boredom/Sleep/Jump-register readouts on the loop-tape render; decide entry-point page on track-mode swap (default Core); add page-nav dot strip in header. Burst remains deferred per Cluster F — no hero page.
- 2026-05-21 (Stochastic UI bar rendering polish): Pitch and duration ticket pages were rendering bars as raw absolute weight (`h = weight * barMaxH / 100`), which lies about engine behavior — `[50,...,50]` and `[100,...,100]` produce identical weighted-random outputs but drew at different heights. Fixed both pages in `StochasticSequenceEditPage.cpp`: aggregate pass computes `maxWeight` and `totalWeight`, bars normalize to `(weight * barMaxH) / max(maxWeight,1)` so uniform rows read flat-top regardless of absolute scale. Faint average-probability hint line drawn at `baseY - avgH` so bars touching the line "pull their weight". Zero-total fallback (engine treats as uniform when all weights == 0) now draws full-height `Color::Low` bars across the row instead of all zero-rects. Excluded `-1` pitch slots keep the X marker. Duration page also got a layout fix for hardware readability: `baseY` raised from 46 to 42 (bar zone y=16..42 was 20..46), label baseline pushed to `baseY+8` from `baseY+6` so the tiny-font label glyph tops no longer crowd the bar bottoms. Commented out the 13 `DBG_STO(...)` trace call sites in `StochasticTrackEngine.cpp` — already macro-neutralized via `#define DBG_STO_ENABLE` guard but visually noisy in source. Committed `75442e8b`.
- 2026-05-21 (Hardware verification pass; Burst math flagged): Phase 2 + two crash fixes confirmed stable on hardware. Save/load round-trips work; persistent loops survive reload; init project / layout commit / project reload combinations no longer crash in any order. Audible feature scenarios mostly check out. One musical design issue surfaced: Burst at parent durations of 1/16 and shorter (1/32, 1/64) produces sub-audible mush even with the 6-tick child gate floor — children pack too tight and merge perceptually with the parent. The current burst math (count = `1 + burstCount * 3 / 100`, spacing = `durationTicks / (count+1) * burstRate/100`) doesn't gracefully degrade at short durations. Flagged for future rework as Cluster F in PHASE10.6-BUG-CATALOG.md: either a hard duration gate (no burst on 1/16 and shorter), a count-shrinking law (children → 1 at short durations), a Burst Probability ramp that drops to zero past a threshold, or a different burst model entirely (e.g., ratchet over the parent instead of independent child events). Not blocking; current behavior is "you can dial it in to taste, just don't use it at 1/16."
