...
### Actionable: Turing as a Tuesday algo — DONE (2026-06-22)
Shipped as Tuesday algo 15 — see `tuesday-turing-algo` (commits `8a42c4b1`/`8e9b053c`/`f9f87512`/`e7a400f7`). Note the survey's "one algo's worth of code" was optimistic: Tuesday implements algos twice (live `TuesdayTrackEngine` + `TuesdayAlgoCore`), so Turing landed in both, sharing a `TuringRegister` core.
