...
| 2 | Engine Container max-size | TeletypeTrackEngine (912 B) inflates all 8 slots | None | Capping Teletype count or separate pool needed | 2,592 B conditional gap vs NoteTrackEngine=588 B; only recoverable if product caps simultaneous Teletype tracks or engine state shrinks below 588 B | Do users actually use >2 Teletype tracks simultaneously? |
...
2. **Model/Engine containers pay max-size tax**. `Container<...>` uses `maxsizeof` to allocate the union. Current ARM probes show `NoteTrack=9544 B` dominates the model container; `TeletypeTrackEngine=912 B` dominates the engine container. Both are genuine requirements for their respective types, but all other types pay the tax unconditionally.
