...
- [ ] **IO / layout / linking: decide linking's scope.** Mapped in `io-layout-linking-map.md`. Linking is a Curve-follows-stepped-source clone — 6 of 8 track types accept inert links. Either (a) stop the UI offering links on types that can't consume/produce, or (b) generalise the `TrackLinkData` contract beyond `SequenceState*` for procedural types (large). Output-layout rationalisation is coupled to the engine-hoist (shared rotation recompute) — do together.
