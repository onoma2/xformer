...
- [ ] **IO / layout / linking map (not yet written).** Map the track-link data flow (`TrackEngine::_linkedTrackEngine`, `Track::_linkTrack`, virtual `linkData()` defaulting to nullptr — only Note/Curve implement it, so linking dead-ends at new track types), physical CV+gate output assignment (`Engine::updateTrackOutputs` gate/CV pools), and `CvInput` routing. Hypothesis: optimized for the Note/Curve-only era; reassess for 8 heterogeneous track types. Sibling to the other two maps; dig in a focused session.
