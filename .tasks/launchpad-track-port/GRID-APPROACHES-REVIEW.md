...
### 1.1 Buffer-Then-Blast MIDI Optimization

**What it does:** All LED changes accumulate in a Lua table (`midigrid.led_buf`) and are sent as **one `midi:send()`** on refresh.
...
**Current performer-phazer approach:** `syncLeds()` iterates all 80 LEDs, compares `_deviceLedState[i] != _ledState[i]`, and sends one MIDI message per changed LED. This is already differential, but each changed LED is a **separate USB-MIDI packet**.

**How we can adopt it:** Buffer all changed LED MIDI bytes into a single array and send one bulk transfer. The STM32 USB stack already supports this via the MIDI TX ring buffer. Instead of:
```cpp
for each changed LED:
    sendMidi(Cable, MidiMessage::makeNoteOn(...))
```
We could build a contiguous byte buffer and send it once:
```cpp
uint8_t buf[256];  // max ~80 LEDs × 3 bytes + SysEx overhead
int len = 0;
for each changed LED:
    len += encodeMidi(buf + len, note, velocity)
mididev.send_bulk(buf, len)
```

**Impact:** Reduces USB transaction overhead. On the STM32F4 USB peripheral, each `sendMidi()` is a separate endpoint packet. Batching cuts IRQ overhead and TX queue pressure.

**Effort:** ~1 hour. Touch only `LaunchpadDevice::syncLeds()`.
