...
## Part 1: The 14 TuesdaySequence Parameters

### **Core Algorithm Control (4 parameters)**

| Parameter | Range | Type | Purpose | RNG Mapping |
|-----------|-------|------|---------|-------------|
| **algorithm** | 0-20 | enum | Algorithm selector | N/A (selector only) |
| **flow** | 0-16 | int | Primary RNG seed, structure control | → `_rng` seed |
| **ornament** | 0-16 | int | Secondary RNG seed, detail control | → `_extraRng` seed |
| **power** | 0-16 | int | Density/cooldown control | → `_coolDownMax` |

### **Loop Control (1 parameter)**

| Parameter | Range | Type | Purpose | Effect |
|-----------|-------|------|---------|--------|
| **loopLength** | 0-29 | int | Finite loop or infinite evolution | 0=infinite, 1-29=steps (maps to 1-64) |
...
| Parameter | Range | Type | Purpose | Effect |
|-----------|-------|------|---------|--------|
| **divisor** | varies | int | Step clock division | 1/4, 1/8, 1/16, 1/32, etc. |
| **resetMeasure** | 0-128 bars | int | Pattern reset interval | 0=never reset, 1-128=bars |

### **Gate Shaping (2 parameters)**

| Parameter | Range | Type | Purpose | Effect |
|-----------|-------|------|---------|--------|
| **gateLength** | 0-100% | int | Gate duration multiplier | Scales algorithmic gateRatio |
| **gateOffset** | 0-100% | int | Gate timing offset | Delays gate from step start |

### **Advanced (3 parameters - less relevant for MINIMAL)**

| Parameter | Range | Type | Purpose | MINIMAL Usage |
|-----------|-------|------|---------|---------------|
| **skew** | -8 to +8 | int | Density curve over loop | ⚠️ Not used in MINIMAL |
| **scan** | 0 to 64-loopLength | int | Buffer window offset | ⚠️ Obsolete (no buffer) |
...
## Part 2: MINIMAL Algorithm - Musical Character
